...
export default function HomePage() {
  ...
  const [summary, setSummary] = useState<SummaryData>({ text: '', loading: false })
  ...
  useEffect(() => {
    ...
    fetch('/api/summary')
      .then((r) => r.json())
      .then((d) => setSummary({ text: d.summary || '', loading: false }))
      .catch(() => setSummary({ text: '', loading: false }))
  ...
  const mockEvents: CalEvent[] = [
    { title: 'Team Standup', startTime: '10:00', endTime: '10:45', tag: 'Work' },
    { title: 'Lunch — James', startTime: '12:30', endTime: '14:00', tag: 'Personal' },
    { title: 'Design Review', startTime: '15:00', endTime: '16:30', tag: 'Work' },
  ]

  const displayEvents = eventsLoaded && events.length > 0 ? events : (!eventsLoaded && session?.accessToken ? [] : mockEvents)

  const eventColor = (tag: string) =>
    tag === 'Work' ? 'var(--sage)' : ACCENT
      ...
      <div style={{ padding: '16px 18px', display: 'flex', flexDirection: 'column', gap: 14 }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
          ...
          <button
            ...
            style={{
              ...
              background: `linear-gradient(135deg, var(--orange), oklch(68% 0.13 28))`,
              border: 'none', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer',
            }}
          >
            {session?.user?.image ? (
              <img src={session.user.image} alt={name} style={{ width: 42, height: 42, borderRadius: '50%', objectFit: 'cover' }} />
            ) : (
        ...
        <LGCard onClick={() => router.push('/weather')} style={{ padding: '16px 18px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
            <span style={{ fontSize: 12, color: 'var(--text-soft)' }}>London, UK</span>
            <div style={{ display: 'flex', alignItems: 'flex-end', gap: 6 }}>
              <span style={{ fontSize: 36, fontWeight: 200, color: 'var(--text)', lineHeight: 1 }}>18°</span>
              <span style={{ fontSize: 13, color: 'var(--text-mid)', marginBottom: 5 }}>Partly Cloudy</span>
            </div>
          </div>
            ...
            <div style={{ display: 'flex', gap: 10 }}>
              {['Mon', 'Tue', 'Wed'].map((d, i) => (
                <div key={d} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 1 }}>
                  <span style={{ fontSize: 9, color: 'var(--text-soft)' }}>{d}</span>
                  <span style={{ fontSize: 11, fontWeight: 500, color: 'var(--text-mid)' }}>{[17, 21, 15][i]}°</span>
                </div>
              ))}
            </div>
          </div>
        ...
        <LGCard style={{ padding: '16px 18px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 10 }}>
            ...
            <span style={{ marginLeft: 'auto', fontSize: 10, color: 'var(--text-soft)' }}>
              {session?.accessToken ? 'AI-generated' : 'Just now'}
            </span>
          </div>
          {summary.loading ? (
            ...
            <p style={{ fontSize: 13.5, color: 'var(--text-mid)', lineHeight: 1.58, fontWeight: 400 }}>
              {summary.text || (
                <>
                  You have <strong style={{ color: 'var(--text)' }}>3 meetings</strong> today starting at 10am.{' '}
                  Sarah sent a proposal — Atlas flagged it as <strong style={{ color: ACCENT }}>high priority</strong>.{' '}
                  Your 5pm gym reminder is set. Weather looks clear for your commute.
                  {!session?.accessToken && (
                    <>{' '}<button onClick={() => router.push('/settings')} style={{ background: 'none', border: 'none', cursor: 'pointer', color: ACCENT, fontSize: 13.5, fontWeight: 500, padding: 0, textDecoration: 'underline' }}>Connect Google</button> for live summaries.</>
                  )}
                </>
              )}
            </p>
        ...
        <LGCard style={{ padding: '16px 18px' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
            ...
            <button
              ...
              style={{ background: 'none', border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 2, color: ACCENT, fontSize: 12, fontWeight: 500 }}
            >
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 11 }}>
            {displayEvents.length === 0 ? (
              <p style={{ fontSize: 13, color: 'var(--text-soft)', textAlign: 'center', padding: '8px 0' }}>No events today</p>
            ) : (
              displayEvents.map((e, i) => (
                <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                  <div style={{ width: 3, height: 38, borderRadius: 4, background: eventColor(e.tag), flexShrink: 0 }} />
                  <div style={{ flex: 1 }}>
                    <div style={{ fontSize: 13, fontWeight: 500, color: 'var(--text)' }}>{e.title}</div>
                    <div style={{ fontSize: 11, color: 'var(--text-soft)' }}>{e.startTime} · {e.tag}</div>
                  </div>
                </div>
              ))
            )}
